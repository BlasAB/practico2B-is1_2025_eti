...
public class App {
   ...
   public static void main(String[] args) {
       ...
       // POST: Maneja el envío del formulario de creación de nueva cuenta (HTML)
       post("/user/new", (req, res) -> {
           ...
           if (name == null || name.isEmpty() || password == null || password.isEmpty()) {
               ...
               res.redirect("/user/create?error=Nombre_y_contraseña_son_requeridos");
           ...
           try {
               ...
               res.redirect("/user/create?error=Error_interno_al_crear_la_cuenta_Intente_de_nuevo");
               return ""; // Retorna una cadena vacía.
           }
       });
       ...
       // Ruta GET para mostrar el formulario de registro de profesor
       get("/profesor/registrar", (req, res) -> {
           ...
           // Si hay mensaje de error, lo agrega al modelo para que Mustache lo muestre
           if (error != null && !error.isEmpty()) {
               model.put("error", error);
           }
           // Si hay mensaje de éxito, lo agrega al modelo para que Mustache lo muestre
           if (successMessage != null && !successMessage.isEmpty()) {
               model.put("successMessage", successMessage);
           }

           // Renderiza la vista profesorForm.mustache con los datos del modelo
           return new ModelAndView(model, "profesorForm.mustache");
       }, new MustacheTemplateEngine());

       // Ruta POST para procesar el registro de un nuevo profesor
        post("/profesor/registrar", (req, res) -> {
            try {
                // Crea un nuevo objeto Profesor y asigna los valores usando los setters
                Profesor p = new Profesor();
                p.setNombre(req.queryParams("nombre"));
                p.setApellido(req.queryParams("apellido"));
                p.setCorreo(req.queryParams("correo"));
                p.setDni(req.queryParams("dni"));
                
                p.saveIt(); // Guarda el profesor en la base de datos

                //Si todo sale bien, redirige al formulario con un mensaje de éxito
                res.redirect("/profesor/registrar?successMessage=Profesor registrado correctamente");
                return null;
            } catch (Exception e) {
                //Si ocurre un error, redirige al formulario con un mensaje de error
       ...
       // Ruta GET para listar todos los profesores registrados
        get("/profesores/listar", (req, res) -> {
           Map<String, Object> model = new HashMap<>();

           // Obtiene todos los registros de la tabla profesores y los pasa al modelo
